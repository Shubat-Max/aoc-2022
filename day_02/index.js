...
/* Part 1 - calculate total score for shape based guide */
const getTotalScoreForShapeBasedGuide = (guide) => {
  return guide
    .replace(/[AX]/g,'1')
    .replace(/[BY]/g,'2')
    .replace(/[CZ]/g,'3')
    ...
    .map(
      row => +row[2] + (
        +row[0] === +row[2] ? 3
          : (+row[0] === 3 && +row[2] === 1) || (+row[0] === 2 && +row[2] === 3) || (+row[0] === 1 && +row[2] === 2) ? 6
            : 0
      )
    )
...
/* Part 2 - calculate total score for outcome based guide */
const getTotalScoreForOutcomeBasedGuide = (guide) => {
  return guide
    .replace(/A/g,'1')
    .replace(/B/g,'2')
    .replace(/C/g,'3')
