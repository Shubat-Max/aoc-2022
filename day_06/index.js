...
const streamMarkerLocator = (stream, charsAmount) => {
  for(let i = 0; i < stream.length - charsAmount; i++){
    const marker = stream.slice(i, i + charsAmount)
    const set = [...new Set(marker.split(''))]
    if(set.length === charsAmount) return stream.indexOf(marker) + charsAmount
  }
  return -1
}

const getPacketMarker = (stream) => streamMarkerLocator(stream, 4);
const getMessageMarker = (stream) => streamMarkerLocator(stream, 14);
...
(async () => {
  ...
  const startOfPacketMarker = getPacketMarker(dataStream)
  console.log(startOfPacketMarker)

  const startOfMessageMarker = getMessageMarker(dataStream)
