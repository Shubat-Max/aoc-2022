...
const getCommonItemsPriorityPerBackpack = (backpacksContent) => {
  return backpacksContent
    ...
    .map(row => {
      ...
      const commonLetters = leftCompartment.filter(letter => rightCompartment.includes(letter))
      const uniqueLetters = [...new Set(commonLetters)]
      const asciiCodes = uniqueLetters.map(l => l.charCodeAt(0))
      return asciiCodes.map(code => code >= 97 ? code - 96 : code - 38)
    })
  ...
  return backpacksContent
    ...
    .reduce((tP, r, i, al) => {
      ...
      const b1 = al[i].split(""),
            b2 = al[i + 1].split(""),
            b3 = al[i + 2].split("")
      return tP += [...new Set(b1.filter(l => b2.includes(l) && b3.includes(l)))]
        .reduce((p, l) => {
          const ac = l.charCodeAt(0)
          return p += ac >= 97 ? ac - 96 : ac - 38
        }, 0)
    }, 0)
}
...
(async () => {
  const backpacksContent = await readFile('input.txt')

  console.time('getCommonItemsPriorityPerBackpack')
  const commonItemsPriorityPerBackpack = getCommonItemsPriorityPerBackpack(backpacksContent) // exec time ~ 1.1 - 1.2 ms.
  console.timeEnd('getCommonItemsPriorityPerBackpack')
  console.log(commonItemsPriorityPerBackpack)

  console.time('getCommonItemsPriorityPerBackpackOL')
  const commonItemsPriorityPerBackpackOL = getCommonItemsPriorityPerBackpackOL(backpacksContent) // exec time ~ 1.6 - 1.7 ms.
  console.timeEnd('getCommonItemsPriorityPerBackpackOL')
  console.log(commonItemsPriorityPerBackpackOL)

  console.time('getCommonItemsPriorityPer3Backpacks')
  const commonItemsPriorityPer3Backpacks = getCommonItemsPriorityPer3Backpacks(backpacksContent) // exec time ~ 0.9 - 1.0 ms.
  console.timeEnd('getCommonItemsPriorityPer3Backpacks')
  console.log(commonItemsPriorityPer3Backpacks)

  console.time('getCommonItemsPriorityPer3BackpacksOL')
  const commonItemsPriorityPer3BackpacksOL = getCommonItemsPriorityPer3BackpacksOL(backpacksContent) // exec time ~ 1.2 - 1.3 ms.
  console.timeEnd('getCommonItemsPriorityPer3BackpacksOL')
  console.log(commonItemsPriorityPer3BackpacksOL)
})()
